# 괄호 검사 프로그램
from stack_class import ArrayStack #모듈 임포트(같은 폴더에 있을 경우)

def checkBrackets(statement):
    # 여는 괄호는 push, 닫는 괄호가 나오면 스택의 맨 위와 동일하면 pop -> LIFO
    ...
    for ch in statement: #입력 문자열 순회
        if ch in openings: # 여는 괄호이면 스택에 push
            ...
            if stack.is_empty(): #조건2 위반 : 짝이 맞지않음
                return False
            if stack.peek() != pairs[ch]: #조건3 위반 : 짝이 맞지 않음
                return False
            stack.pop()
        else:
            pass # 괄호가 아니면 무시
