   ...
   # 원형덱에서만 있는 기능 : 자식 클래스 구현
    def add_front(self, item): # 전단에서 item 삽입
        if self.is_full():
            ...
            self.front = (self.front - 1 + self.N) % self.N # front를 반시계방향으로 한칸 이동

    def delete_rear(self): # 후단에서 삭제
        if self.is_empty():
            raise IndexError("원형덱이 빈상태 -> 삭제불가")   
        item = self.array[self.rear] # 가장 최근에 저장된 데이터 가리킴
        self.array[self.rear] = None # (option)
        self.rear = (self.rear - 1 + self.N) % self.N # rear를 반시계방향으로 한칸 이동
        return item
    
    def get_rear(self): # 위쪽에서 최근 저장된 데이터 조회
        if self.is_empty():
