...
def is_leaf(node): # 노드가 단말(리프)노드 확인하기
	if node is None:
		return False
...
def print_leaf_nodes(node): # 이진트리의 단말노드만 출력하기
	...
	if node is None: # 재귀호출의 종료
		return 
	if is_leaf(node): # 단말노드이면
		print_data(f"{node.data}: 단말노드")
		return
	#리프노드가 아니면 재귀호출'
...
def levelorder(root): # 레벨순회
	...
	while q: # 큐가 공백이 될 때까지 반복 처리
		...
		if node.left: # 현재 노드의 왼쪽 자식노드가 존재하는 경우
			q.append(node.left) # 큐에 삽입
		if node.right: # 현재 노드의 오른쪽 자식노드가 존재하는 경우
			q.append(node.right) # 큐에 삽입

def count_nodes(root): # 이진트리의 노드 수 = 왼쪽 서브트리의 노드 수 + 오른쪽 서브트리의 노드 수 + 1
	# 재귀적 정의 : 트리가 비어있는 경우 0, 아니면 왼쪽과 오른쪽 서브트리의 노드 수 합 + 1
	if root is None:
		return 0
	return count_nodes(root.left) + count_nodes(root.right) + 1

def count_leaves(node): # 이진트리의 단말노드 수 = 왼쪽 서브트리의 단말노드 수 + 오른쪽 서브트리의 단말노드 수
	# 재귀적 정의 : 트리가 비어있는 경우 0, 리프노드이면 1, 아니면 왼쪽과 오른쪽 서브트리의 단말노드 수 합산
	if node is None:
		return 0
	if is_leaf(node):
		return 1
...
def tree_height(root): # 이진트리의 높이
	# 재귀적 정의 : 트리가 빈 경우 0, 아니면 왼쪽 서브트리의 높이와 오른쪽 서브트리의 높이의 최대값 + 1
	if root is None:
		return 0
...
def count_edges(root): # 이진트리의 간선(연결된 링크) 의 수
	...
	node = count_nodes(root) # 트리의 노드 수
	return max(0, node - 1) # 노드 개수가 0인 ㄱㅇ우 음수가 나오지 않도록

def min_nodes_bitree(k): # 높이가 K인 이진트리의 최소 노드의 수(루프레벨 1)
...
def max_nodes_bitree(k): # 높이가 K인 이진트리의 최대 노드의 수(루프레벨 1)
