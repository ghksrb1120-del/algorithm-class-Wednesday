...
def string_matching_bf(text, pattern):
    ...
    for i in range(n-m+1): # 비교가능한 마지막 위치는 n - m
        ...
        if j == m: #패턴의 모든 문자가 일치
            return i # 일치하는 부분 문자열의 시작 위치 변환
...
def string_matching_all_bf(text, pattern):
    ...
    for i in range(n-m+1): # 비교가능한 마지막 위치는 n - m
        ...
        while j < m and pattern[j] == text[i + j]:
            j += 1 # 다음 문자로 이동
        if j == m: #패턴의 모든 문자가 일치
            matches.append(i) # 일치하는 부분 문자열의 텍스트의 시작 위치 추가
    return matches # 일치하는 부분 문자열이 없는 경우

# 테스트 
text = "0101010101010101010101010"
pattern = "1010"
...
def knapsack01_bf(wgt, val, W):
    ...
    n = len(wgt) # 물건의 개수
    bestVal = 0 # 최대 가치 초기화
    bestSet = [] # 최대 가치를 제공하는 부분 집합 조합 기록
    count = 0 # 부분집합의 번호 표시 용도

    # 0 ~ 2^n - 1 모든부분집합 조합 탐색
    for i in range(2 ** n):
        count += 1
        ...
        for j in range(n):
            if s[j] == 1:
                sumWgt += wgt[j]
                sumVal += val[j]
                chosen_items.append(j) # 선택된 j 번째 물건의 인덱스 추가
...
coins = [500, 100, 60, 50, 10, 5, 1] # 실제 최적해는 동전 3개 사용 경우 : (500, 1), (60, 2)
amount = 620
...
# - 그리디 알고리즘 - 항상 최적해 보장
def fracknapsack_greddy(weights, values, W):
    ...
    for ratio, wgt, val, idx in items: # 비율이 높은 순서로 물건 서택
        if W <= 0: # 가방이 꽉 찬 경우
            break
...
values = [120, 80, 60]
W = 18
bestVal, bag_with_items = fracknapsack_greddy(weights, values, W)
